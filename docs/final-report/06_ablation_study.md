...
|---|---|---|---|---|
| kp5  | 0,5,6,11,12 | 21 | 0.8728 | vw=5,vk=3 (불안정) |
...
- kp5는 21개 특징으로 최저 — ev_minpr=0.8728 달성하나 postproc 파라미터(v5k3)가 불안정하고 val/test 편차 크음
...
## 6.12 추가 분석: 카메라별 성능 변동 (Experiment 1 결과)

ablation 최상위 모델들을 카메라별로 평가하여 성능 일반성 확인.

| 카메라 | 비디오 | P42-kp17-w60 | P41-kp13-w60 | P41-raw-kp7-w40 |
|---|---|---|---|---|
| C5 | 212 | 0.9492 | 0.9322 | 0.9492 |
| C6 | 220 | 0.9388 | **0.9600** | 0.9038 |
| C7 | 213 | 0.8846 | 0.8846 | 0.8462 |
| C8 | 208 | 0.9118 | 0.8824 | 0.9118 |

**주요 발견**: C7 후면 카메라에서 모든 모델 성능 저하 (0.8462~0.8846). 낙상 방향별 recall은 뒤(BY)≥0.995로 가장 높고, 앞(FY) recall이 상대적으로 낮음(0.965~0.983).

---

## 6.13 추가 분석: Keypoint 신뢰도 vs 성능

같은 모델이라도 입력 keypoint의 신뢰도에 따라 성능이 크게 달라짐.

| 신뢰도 분위 | P42-kp17-w60 MinP | 핵심 지표 |
|---|---|---|
| Q1 (≤0.371) | 0.450 | NFallPrecision=0.450 — FP 급증 |
| Q2 (0.371–0.425) | 0.860 | |
| Q3 (0.425–0.504) | 0.941 | |
| Q4 (>0.504) | 0.980 | FP=0, 완벽에 근접 |

전체 ev_minpr(0.9211)은 신뢰도 분포 혼합의 평균. 고신뢰도 환경에서는 훨씬 높은 성능 달성 가능.
