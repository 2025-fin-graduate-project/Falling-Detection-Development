#!/usr/bin/env bash
# TCN ablation: filter × kp × window
# Filter A (raw), EMA (splits_v2_ema), D (filtered)
# KP: kp7, kp12, all
# Window: 20, 30, 40, 50
# Model: TCN default [32,32,64,96] dil=[1,2,4,8] k=3
# (mirrors pure_gru_ablation axes for direct comparison)

set -euo pipefail
REPO="$(cd "$(dirname "$0")/.." && pwd)"
cd "$REPO"

OUT="$REPO/results/tcn_ablation"
LOG="$OUT/run.log"
mkdir -p "$OUT"
...
# 공통 하이퍼파라미터 (pure_gru_ablation과 동일)
COMMON="
  --model-type tcn
...
print(f"{'ID':<22} {'MinPR':>6} {'F1':>6} {'Recall':>7}")
print("-" * 45)
for r in rows:
