#!/usr/bin/env python3
"""Build splits_v2_filtered_kv: PP-D filtered kp + HSSC/RWHC/VHSSC/AHSSC + per-keypoint velocity.

Extends the base filtered pipeline by computing per-joint velocity (vy, vx)
for the 5 fall-relevant joints: nose(0), shoulders(5,6), hips(11,12).
These velocities capture individual limb motion that center-of-mass VHSSC misses.

Output: dataset/splits_v2_filtered_kv/{train,val,test}.csv
Feature additions vs splits_v2_filtered:
  kp0_vy, kp0_vx, kp5_vy, kp5_vx, kp6_vy, kp6_vx, kp11_vy, kp11_vx, kp12_vy, kp12_vx

Usage:
    uv run python scripts/util/build_filtered_v2_splits_kv.py
"""
...
# Fall-relevant joints for per-joint velocity: nose, L/R shoulders, L/R hips
...
def _process_video(group: pd.DataFrame,
    ...
    # 4. HSSC / RWHC center-of-mass features
    ...
    # 7. Per-keypoint velocity for fall-relevant joints
    for i in KV_KP_IDX:
        if f"kp{i}_y" in g.columns:
            g[f"kp{i}_vy"] = np.gradient(g[f"kp{i}_y"].values, times)
        if f"kp{i}_x" in g.columns:
            g[f"kp{i}_vx"] = np.gradient(g[f"kp{i}_x"].values, times)
...
def process_split(src: Path, dst: Path) -> None:
    ...
    kv_cols = [c for c in out.columns if c.endswith("_vy") or c.endswith("_vx")]
    print(f"    → {dst}  (normal={n0:,}  falling={n1:,}  fallen={n2:,}  vel_cols={len(kv_cols)})")
...
def main() -> None:
    ...
    dst_dir = root / "splits_v2_filtered_kv"
